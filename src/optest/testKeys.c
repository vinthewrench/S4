...
static C4Err  sTestECCKeys()
{
    ...
    OPTESTLogInfo("\nTesting C4 ECC KeyEncoding \n");
    
    OPTESTLogDebug("\n\tImport ECC key\n");
    err = ECC_Init(&eccPub);
    err = ECC_Import_ANSI_X963( eccPub, ecc414_pubkey, sizeof(ecc414_pubkey));CKERR;
    
    OPTESTLogDebug("\tImport TBC key\n");
     err = C4Key_NewTBC(kTBC_Algorithm_3FISH512, three_512_key, &keyCtx  ); CKERR;
     
    OPTESTLogDebug("\tEncode TBC key\n");
   err = C4Key_SerializeToPubKey(keyCtx, eccPub, &data, &dataLen); CKERR;
    
    OPTESTLogDebug("\n------\n%s------\n",data);
    
    OPTESTLogDebug("\tInport TBC key\n");
